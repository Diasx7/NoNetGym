...
router.get('/', (req, res) => {
  db.query(
    'SELECT * FROM exercises WHERE user_id = ? ORDER BY exercise_date DESC, created_at DESC',
    [req.userId],
    (err, results) => {
...
router.post('/', (req, res) => {
  ...
  db.query(
    'INSERT INTO exercises (user_id, name, sets, reps, weight, exercise_date) VALUES (?, ?, ?, ?, ?, ?)',
