...
router.get('/', (req, res) => {
  db.query(
    'SELECT * FROM measures WHERE user_id = ? ORDER BY measure_date DESC',
    [req.userId],
    (err, results) => {
...
router.post('/', (req, res) => {
  ...
  db.query(
    'SELECT id FROM measures WHERE user_id = ? AND measure_date = ?',
    ...
    (err, results) => {
      ...
      if (results.length > 0) {
        ...
        db.query(
          'INSERT INTO measures (user_id, weight, waist, abdomen, chest, arm, leg, measure_date) VALUES (?,?,?,?,?,?,?,?)',
