async function atualizarDashboard() {
  ...
  document.getElementById('peso-atual').innerHTML = (stats.peso || '—') + '<span style="font-size:16px">kg</span>'
  ...
  el.innerHTML = `
    ...
    ${doUltimoDia.map(ex => `
      <div style="font-size:13px;color:var(--cinza);padding:2px 0">
        ${ex.name} — ${ex.sets}x${ex.reps} · ${ex.load}kg
      </div>
    `).join('')}
  `
}
...
async function renderizarProgresso() {
  ...
  document.getElementById('p-sequencia').textContent = stats.sequencia + '🔥'
  document.getElementById('p-peso').textContent = stats.peso || '—'
...
function renderizarConquistas(stats, medidas) {
  const conquistas = [
    { icone: '🏅', texto: 'Primeiro treino!',         ganhou: stats.total >= 1 },
    { icone: '🔥', texto: '7 treinos no total',        ganhou: stats.total >= 7 },
    { icone: '⚡', texto: '3 dias seguidos',            ganhou: stats.sequencia >= 3 },
    { icone: '📏', texto: 'Medidas registradas',        ganhou: medidas.length >= 1 },
    { icone: '🏆', texto: '10 treinos completados',     ganhou: stats.total >= 10 },
    { icone: '💪', texto: '30 treinos — você é faixa!', ganhou: stats.total >= 30 },
  ]
  ...
  document.getElementById('lista-conquistas').innerHTML = conquistas.map(c => `
    <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--borda);opacity:${c.ganhou ? 1 : 0.3}">
      ...
      ${c.ganhou ? '<span style="margin-left:auto;color:var(--verde);font-size:12px">✓</span>' : ''}
    </div>
  `).join('')
}